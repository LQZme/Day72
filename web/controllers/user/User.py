...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.login_pwd, passwd):
            session['isLogged'] = 1
...
def index(page=None):
    ...
    if keyword:
        users = User.query.filter(User.login_name.contains(keyword)).order_by(User.id)\
            .paginate(page, per_page=app.config['PAGE_SIZE'])
        params['total'] = users.total
        pagebanner = iPagination(params)
        condition = "?search=" + keyword
        return render_template('/user/index.html', users=users, condition=condition, pages=pagebanner)
    else:
...
def add():
    ...
    if form.validate_on_submit():
        try:
            user = User(form.login_name.data,
                        form.login_pwd.data,
                        form.status.data)
            db.session.add(user)
            db.session.commit()
...
@user_route.route('/delete/<int:pk>')
def delete(pk):
    user = User.query.get(pk)
    if not user:
        return redirect(url_for('.index'))
